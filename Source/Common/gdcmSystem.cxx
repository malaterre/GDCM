...
#if 1
  char *p, *s;
  if (sp == GDCM_NULLPTR || *sp == GDCM_NULLPTR || **sp == '\0') return GDCM_NULLPTR;
  s = *sp;
  p = s + strcspn(s, sep);
  if (*p != '\0') *p++ = '\0';
  *sp = p;
...
{
  const char *codeset = GDCM_NULLPTR;
