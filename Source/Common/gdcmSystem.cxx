...
#else
inline int Mkdir(const char *dir) { return mkdir(dir, 00777); }
...
//    }
  ...
  static char path[PATH_MAX];
  if (readlink("/proc/self/exe", path, sizeof(path)) >
      0)  // Technically 0 is not an error, but that would mean
  ...
  const char *System::GetCurrentResourcesDirectory() {
    ...
    if (pathURL != nullptr) {
      success =
          CFURLGetFileSystemRepresentation(pathURL, true /*resolveAgainstBase*/,
...
#define DELTA_EPOCH_IN_MICROSECS 11644473600000000ULL
  ...
  int gettimeofday(struct timeval * tv, struct timezone * tz) {
    /*
       ...
       should normally  be  specified  as  NULL.  The tz_dsttime field has never
       been used under Linux; it has not been and will not be supported by libc
       or glibc.   Each  and  every occurrence of this field in the kernel
       source (other than the declaration) is a bug. Thus, the following is
  ...
  /**
   Implementation note. We internally use mktime which seems to be quite relaxed
   when it comes to invalid date. It handles : "17890714172557";
   "19891714172557"; "19890014172557"; While the DICOM PS 3.5-2008 would
   prohibit them. I leave it this way so that we correctly read in /almost/
   valid date. What we write out is always valid anyway which is what is
   important.
  */
  ...
  bool System::ParseDateTime(time_t & timep, long &milliseconds,
                             const char date[22]) {
    ...
    if ((n = sscanf(date, "%4d%2d%2d%2d%2d%2d", &year, &mon, &day, &hour, &min,
                    &sec)) >= 1) {
      switch (n) {
        case 1:
          mon = 1;
    ...
    if (len > 14)  // more data to process
    {
      ...
      if (!*ptr || sscanf(ptr, "%06ld", &milliseconds) != 1) {
        // Could not parse milliseconds but date looks ok, should I return false
        // anyway ?
  ...
  bool System::GetCurrentDateTime(char date[22]) {
    ...
    // Apparently suseconds_t is defined as long on linux system... why would
    // this be signed ?
...
// function.
  ...
  static const char *CharsetAliasToName(const char *alias) {
    ...
    gdcmWarningMacro(std::string("Could not find Charset from alias: ") +
                     alias);
    return NULL;
  }
#endif  //_WIN32
