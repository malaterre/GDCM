...
Surface::STATES Surface::GetSTATES(const char * state)
{
  ...
  for(unsigned int i = 0; STATESStrings[i] != GDCM_NULLPTR; ++i)
  {
...
Surface::VIEWType Surface::GetVIEWType(const char * type)
{
  ...
  for(unsigned int i = 0; VIEWStrings[i] != GDCM_NULLPTR; ++i)
  {
...
Surface::Surface():
  ...
  PointPositionAccuracy(GDCM_NULLPTR),
  MeanPointDistance(0),
  MaximumPointDistance(0),
  PointsBoundingBoxCoordinates(GDCM_NULLPTR),
  AxisOfRotation(GDCM_NULLPTR),
  CenterOfRotation(GDCM_NULLPTR),
  NumberOfVectors(0),
  VectorDimensionality(0),
  VectorAccuracy(GDCM_NULLPTR),
...
Surface::~Surface()
{
  if (PointPositionAccuracy != GDCM_NULLPTR)         delete PointPositionAccuracy;
  if (PointsBoundingBoxCoordinates != GDCM_NULLPTR)  delete PointsBoundingBoxCoordinates;
  if (AxisOfRotation != GDCM_NULLPTR)                delete AxisOfRotation;
  if (CenterOfRotation != GDCM_NULLPTR)              delete CenterOfRotation;

  if (VectorAccuracy != GDCM_NULLPTR)                delete VectorAccuracy;
}
...
void Surface::SetPointPositionAccuracy(const float * accuracies)
{
  ...
  if (PointPositionAccuracy == GDCM_NULLPTR) PointPositionAccuracy = new float[3];
...
void Surface::SetPointsBoundingBoxCoordinates(const float * coordinates)
{
  ...
  if (PointsBoundingBoxCoordinates == GDCM_NULLPTR) PointsBoundingBoxCoordinates = new float[6];
...
void Surface::SetAxisOfRotation(const float * axis)
{
  ...
  if (AxisOfRotation == GDCM_NULLPTR) AxisOfRotation = new float[3];
...
void Surface::SetCenterOfRotation(const float * center)
{
  ...
  if (CenterOfRotation == GDCM_NULLPTR) CenterOfRotation = new float[3];
...
void Surface::SetVectorAccuracy(const float * accuracy)
{
  ...
  if (VectorAccuracy == GDCM_NULLPTR) VectorAccuracy = new float[ VectorDimensionality ];
