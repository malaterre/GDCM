...
{
//-----------------------------------------------------------------------------
PrinterJSON::PrinterJSON():PrintStyle(PrinterJSON::VERBOSE_STYLE),F(0)
{
...
void PrinterJSON::SetColor(bool c)
{
  if( c )
    {
...
void PrintValueJSON(VR::VRType const &vr, VM const &vm, const Value &v);
...
//-----------------------------------------------------------------------------
  /*
void PrinterJSON::PrintElement(std::ostream& os, const ImplicitDataElement &ide, DictEntry const &entry)
{
  ...
  if( _val.GetVL() )
    ...
    if( dictVR != VR::INVALID && VR::IsBinary(dictVR) )
      {
      PrintValueJSON(dictVR, vm, value);
      }
    else
      {
      _os << "[" << value << "]";
...
void PrintValueJSON(VR::VRType const &vr, VM const &vm, const Value &v)
{
  try
  ...
  catch(...)
    {
      ...
      std::cerr << "Problem in PrintValueJSON" << std::endl;
    }
}

//-----------------------------------------------------------------------------
#if 0
void PrinterJSON::PrintDataSet(std::ostream& os, const DataSet<ImplicitDataElement> &ds)
{
  //ImplicitDataElement de;
  PrinterJSON::PrintStyles pstyle = is.GetPrintStyle();
  (void)pstyle;
...
#define StringFilterCase(type) \
  case VR::type: \
    { \
      ...
      if(!isFloat(s)) os<<"\""<<el.GetValue()<<"\"";\
      else\
        os<<el.GetValue();\
      for(long i = 1; i < l; ++i){\
        std::stringstream temp;\
        temp << el.GetValue((unsigned int)i);\
        std::string s = temp.str();\
        if(!isFloat(s)) os<<",\""<<el.GetValue((unsigned int)i)<<"\"";\
        else\
...
bool PrinterJSON::isFloat( std::string& s ) const {
...
VR PrinterJSON::PrintDataElement(std::ostringstream &os, const Dicts &dicts, const DataSet & ds,
  const DataElement &de, std::ostream &out, std::string const & indent )
{
  ...
  os<<"\"(";
  os <<std::hex<< std::setw( 4 ) << std::setfill( '0' )<< t[0]<< std::dec;
  os<< ", ";
  os<< std::hex << std::setw( 4 ) << std::setfill( '0' ) << t[1]<< std::dec;
  os<<")\" : ";
  //os <<t[];
  ...
  if( refvr & VR::VRASCII )
    ...
    case VR::INVALID:
        ...
        if( bv )
          ...
          else if( t == Tag(0xfffe,0xe000) ) bv->PrintASCIIJSON(os);
          else
            {
            bv->PrintASCIIJSON(os);
            }
...
void PrinterJSON::PrintDataSet(const DataSet &ds, std::ostream &out, std::string const & indent )
{
  ...
  for( ; it != ds.End(); ++it )
    ...
    DataSet::ConstIterator tempit = it;
    tempit++;
    if(tempit != ds.End()){
        os << ","<<std::endl;
    }
...
//-----------------------------------------------------------------------------
void DumpDataSetJSON(const DataSet &ds, std::ostream &os )
{
  DataSet::ConstIterator it = ds.Begin();
  for( ; it != ds.End(); ++it )
    {
...
//-----------------------------------------------------------------------------
void PrinterJSON::Print(std::ostream& os)
{
