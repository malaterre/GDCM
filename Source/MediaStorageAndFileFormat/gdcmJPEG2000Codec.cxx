...
{
  opj_stream_t* l_stream = GDCM_NULLPTR;
  if
    (! p_mem)
  {
    return GDCM_NULLPTR;
  ...
  opj_stream_set_user_data(l_stream,p_mem,GDCM_NULLPTR);
...
std::pair<char *, size_t> JPEG2000Codec::DecodeByStreamsCommon(char *dummy_buffer, size_t buf_size)
{
  ...
  opj_codec_t* dinfo = GDCM_NULLPTR;  /* handle to a decompressor */
  opj_stream_t *cio = GDCM_NULLPTR;
  opj_image_t *image = GDCM_NULLPTR;
  ...
  bResult = bResult && (image != GDCM_NULLPTR);
...
bool JPEG2000Codec::CodeFrameIntoBuffer(char * outdata, size_t outlen, size_t & complen, const char * inputdata, size_t inputlength )
{
  ...
  if(parameters.cp_comment == GDCM_NULLPTR) {
  ...
  opj_codec_t* cinfo = GDCM_NULLPTR;
  opj_stream_t *cio = GDCM_NULLPTR;
...
bool JPEG2000Codec::GetHeaderInfo(const char * dummy_buffer, size_t buf_size, TransferSyntax &ts)
{
  ...
  opj_set_error_handler(dinfo, gdcm_error_callback, GDCM_NULLPTR);
