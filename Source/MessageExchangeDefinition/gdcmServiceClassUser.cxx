...
public:
  ...
  ServiceClassUserInternals():mConnection(GDCM_NULLPTR),mSecondaryConnection(GDCM_NULLPTR){}
...
bool ServiceClassUser::StartAssociation()
{
  ...
  ULEvent theEvent(eAASSOCIATERequestLocalUser, GDCM_NULLPTR);
  network::EStateID theState = RunEventLoop(theEvent, Internals->mConnection, GDCM_NULLPTR, false);
  if(theState != eSta6TransferReady)
    ...
    for( std::vector<BasePDU*>::const_iterator itor
      ...
      {
      assert(*itor);
      if (*itor == GDCM_NULLPTR) continue; //can have a nulled pdu, apparently
...
bool ServiceClassUser::StopAssociation()
{
  ...
  EStateID theState = RunEventLoop(theEvent, mConnection, GDCM_NULLPTR, false);
  ...
  const bool& startWaiting = false)
{
  ...
  do {
    ...
    BasePDU* theFirstPDU = GDCM_NULLPTR;// the first pdu read in during this event loop,
          ...
          case ePDATATFPDU:
            ...
            //if (theState == eSta6TransferReady){//ie, finished the transitions
              ...
              if (theVal != pendingDE1 && theVal != pendingDE2 && theVal != success)
                ...
                const ByteValue *err1 = GDCM_NULLPTR, *err2 = GDCM_NULLPTR;
              ...
              if (theVal == pendingDE1 || theVal == pendingDE2 /*|| theVal == success*/){//keep looping if we haven't succeeded or failed; these are the values for 'pending'
                ...
                do {
                  ...
                  if (itemtype != 0x4 && thePDU != GDCM_NULLPTR){ //ie, not a pdatapdu
...
EStateID ServiceClassUser::RunMoveEventLoop(ULEvent& currentEvent, ULConnectionCallback* inCallback){
  ...
  do {
    ...
    if (Internals->mSecondaryConnection->GetProtocol() == GDCM_NULLPTR){
    ...
    if (secondConnectionEstablished &&
      ...
      ULEvent theCStoreEvent(eEventDoesNotExist, GDCM_NULLPTR);//have to fill this in, we're in passive mode now
