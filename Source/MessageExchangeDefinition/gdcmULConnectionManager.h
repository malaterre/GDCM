 ...
 */
  class GDCM_EXPORT ULConnectionManager : public Subject
  {
    ...
    public:
      ...
      // \internal
      // API will change...
      std::vector<DataSet> SendStore(const File &file, std::istream * pStream = GDCM_NULLPTR, std::streampos dataSetOffset = 0  );
      ...
      ///callback based API
      void SendStore(const File & file, ULConnectionCallback* inCallback, std::istream * pStream = GDCM_NULLPTR , std::streampos dataSetOffset = 0 );
