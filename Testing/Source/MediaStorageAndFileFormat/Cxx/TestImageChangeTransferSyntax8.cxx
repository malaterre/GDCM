...
int TestImageChangeTransferSyntaxJPEGTurboLossless(const char *filename, bool verbose = false)
{
  ImageReader reader;
  ...
  const char subdir[] = "TestImageChangeTransferSyntax8";
...
int TestImageChangeTransferSyntax8(int argc, char *argv[])
{
  if( argc == 2 )
    {
    const char *filename = argv[1];
    return gdcm::TestImageChangeTransferSyntaxJPEGTurboLossless(filename, true);
    }
  ...
  while( (filename = filenames[i]) )
    {
    r += gdcm::TestImageChangeTransferSyntaxJPEGTurboLossless( filename );
    ++i;
    }
