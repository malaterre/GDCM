...
void display_element(std::ostream& os, const gdcm::DataElement& de, const gdcm::DictEntry& dictentry, const char *note, int depthSQ);
void underline(int depthSQ);
void difference_of_sequences(const gdcm::DataElement& sqde1, const gdcm::DataElement& sqde2, const gdcm::DictEntry& dictentry, int depthSQ);
...
int main( int argc, const char* argv[] ) {
  ...
  if (3>argc) {
    std::cerr <<  "\nMust supply the filenames of 2 DICOM files\n" << std::endl;
    usage();
    return 1;
  }
  ...
  for(int i=1; i<(argc-2) ;i++) {
    ...
    if ("-h"==arg || "--help"==arg) {
      ...
      std::cerr << "Warning: command argument not understood: " << arg << std::endl ;
    }
  }
  // Start comparison
  ...
  if(include_meta) difference_of_datasets(hds1, hds2, 0);
...
void difference_of_datasets(const gdcm::DataSet& ds1, const gdcm::DataSet& ds2, int depthSQ) {
  ...
  do {
    ...
    if(tag1==tag2) {
      if(!(*it1==*it2)) {// != op is not defined for DataElement in GDCM 2.0.17 // TODO: can this logical iterator spaghetti be tidied up?
      ...
      if (it2!=ds2.End()) it2++;
    } else { // if tags out of sync, only increment the iterator that is behind. this happens when a tag is only in one set
      if (it1==ds1.End()) it2++;
      else if (it2==ds2.End())it1++;
      else {
        if (tag1<tag2) it1++;
        else it2++;
      }
...
void display_element(std::ostream& os, const gdcm::DataElement& de, const gdcm::DictEntry& dictentry, const char * note, int depthSQ){
  ...
  if (vr & gdcm::VR::VRBINARY) {
    ...
    if (de.GetByteValue() != NULL) { // is this OK? it worked when de was a pointer, without the != NULL
      ...
      if (0==Truncate) trimto= de.GetByteValue()->GetLength();
...
void difference_of_sequences(const gdcm::DataElement& sqde1, const gdcm::DataElement& sqde2, const gdcm::DictEntry& dictentry, int depthSQ){
  ...
  if (n1!=n2) sq_note << "[sequence, file 1 has " << n1 << " datasets, file 2 has " << n2 << " datasets]";
  else sq_note << "[sequence]";
  display_element(sq_disp, sqde1, dictentry, sq_note.str().c_str(),depthSQ);
  for(int i=1; i<=n ;i++) {
    difference_of_datasets( sqi1->GetItem(i).GetNestedDataSet(), sqi2->GetItem(i).GetNestedDataSet(), depthSQ+1);
  }
