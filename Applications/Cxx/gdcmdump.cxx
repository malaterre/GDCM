...
{
  // 01f7,1026
  const gdcm::ByteValue *bv2 = GDCM_NULLPTR;
...
static int PrintMrProtocol(const std::string & filename)
{
  ...
  else if( ds.FindDataElement( sfgs.GetTag() ) )
  {
    ...
    if( sqi != GDCM_NULLPTR && sqi->GetNumberOfItems() == 1 ) {
      ...
      if( subds.FindDataElement( att2) ) {
        ...
        if( sqi2 != GDCM_NULLPTR && sqi2->GetNumberOfItems() == 1 ) {
...
*/
  ...
  const char * csaname = GDCM_NULLPTR;
  if( printcsaasl )
  {
    printcsabase64 = 1;
    csaname = "MR_ASL";
