...
int x509write_crtfile(x509_raw *chain, unsigned char *path, int out_flag)
{
    ...
    return x509write_file(&chain->raw, path, out_flag, cer_beg, cer_end);
}
...
static int x509write_parse_names(x509_node *node, unsigned char *names)
{
    ...
    int C_len, CN_len, O_len, OU_len, ST_len, L_len, R_len;
    ...
    /* email */
    if (R != NULL) {
        if ((ret = x509write_add_name(node, OID_PKCS9_EMAIL, 9, R, R_len,
                        ASN1_IA5_STRING)) != 0)
            return ret;
    }
...
static int x509write_make_sign(x509_raw *chain, rsa_context *privkey)
{
     ...
     */
    return asn1_add_oid(&chain->signalg, OID_PKCS1_RSA_SHA, 9, 
                  ASN1_CONSTRUCTED | ASN1_SEQUENCE, ASN1_NULL, "", 0);
}
...
int x509write_create_csr(x509_raw *chain, rsa_context *privkey)
{
    ...
    /* write attributes */
    if ((ret = asn1_add_obj("", 0, ASN1_CONTEXT_SPECIFIC | 
