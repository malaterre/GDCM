...
vtkLookupTable16::vtkLookupTable16(int sze, int ext)
{
  ...
  this->Table = vtkUnsignedShortArray::New();
...
int vtkLookupTable16::IsOpaque()
{
  if(this->OpaqueFlagBuildTime<this->GetMTime())
    ...
    int i=0;
    unsigned short *ptr=this->Table->GetPointer(0);
    while(opaque && i<size)
      {
      opaque=ptr[3]==255;
      ptr+=4;
...
// range value.
void vtkLookupTable16::SetTableRange(double r[2])
{
...
// Scalar values greater than maximum range value are clamped to maximum
// range value.
void vtkLookupTable16::SetTableRange(double rmin, double rmax)
{
...
void vtkLookupTable16::SetScale(int scale)
{
  if (this->Scale == scale)
    {
    return;
...
int vtkLookupTable16::Allocate(int sz, int ext)
{
...
void vtkLookupTable16::ForceBuild()
{
  int i;
  double hue, sat, val, hinc, sinc, vinc, ainc;
  double rgba[4], alpha;
  unsigned short *c_rgba;
...
void vtkLookupTable16::GetColor(double v, double rgb[3])
{
...
double vtkLookupTable16::GetOpacity(double v)
{
  unsigned char *rgb8 = this->MapValue(v);
...
void vtkLookupTable16LogRange(double range[2], double logRange[2])
{
  double rmin = range[0];
  double rmax = range[1];
...
vtkIdType vtkLookupTable16::GetIndex(double v)
{
  ...
  if (this->Scale == VTK_SCALE_LOG10)
    ...
    double logRange[2];
    vtkLookupTable16LogRange(this->TableRange, logRange);
    shift = -logRange[0];
...
void vtkLookupTable16::SetTable(vtkUnsignedShortArray *table)
{
...
unsigned char *vtkLookupTable16::MapValue(double v)
{
                           ...
                           int inIncr, int outFormat)
{
  ...
  if ( (alpha=self->GetAlpha()) >= 1.0 ) //no blending required 
    ...
    if (self->GetScale() == VTK_SCALE_LOG10)
      ...
      double val;
      double logRange[2];
      vtkLookupTable16LogRange(range, logRange);
      shift = -logRange[0];
      if (logRange[1] <= logRange[0])
        {
        scale = VTK_DOUBLE_MAX;
...
template<class T>
void vtkLookupTable16MapMag(vtkLookupTable16 *self, T *input, 
                          unsigned char *output, int length, 
                          int inIncr, int outFormat)
{
  double tmp, sum;
  ...
  vtkLookupTable16MapData(self, mag, output, length, 1, outFormat);
                                             ...
                                             int outputFormat)
{
  if (this->UseMagnitude && inputIncrement > 1)
    ...
    switch (inputDataType)
      {
      vtkTemplateMacro(
        vtkLookupTable16MapMag(this,static_cast<VTK_TT*>(input),output,
  ...
  switch (inputDataType)
    ...
    case VTK_BIT:
      ...
      vtkUnsignedShortArray *newInput = vtkUnsignedShortArray::New();
    ...
    vtkTemplateMacro(
      vtkLookupTable16MapData(this,static_cast<VTK_TT*>(input),output,
                            numberOfValues,inputIncrement,outputFormat)
      );
    default:
      vtkErrorMacro(<< "MapImageThroughTable: Unknown input ScalarType");
...
void vtkLookupTable16::SetTableValue(vtkIdType indx, double rgba[4])
{
  ...
  unsigned short *_rgba = this->Table->WritePointer(4*indx,4);
...
void vtkLookupTable16::SetTableValue(vtkIdType indx, double r, double g, double b, 
                                   double a)
{
  double rgba[4];
...
void vtkLookupTable16::GetTableValue(vtkIdType indx, double rgba[4])
{
  unsigned short *_rgba;
...
// Return a rgba color value for the given index into the lookup table. Color
// components are expressed as [0,1] double values.
double *vtkLookupTable16::GetTableValue(vtkIdType indx)
{
...
//----------------------------------------------------------------------------
void vtkLookupTable16::DeepCopy(vtkLookupTable16 *lut)
{
  if (!lut)
    {
    return;
