...
void sockinetaddr::setport(const char* sn, const char* pn)
{
    if (isdigit (*sn)) {
      ...
      if (sp == GDCM_NULLPTR) throw sockerr (EADDRNOTAVAIL, "sockinetaddr::setport");
      sin_port = sp->s_port;
    }
}
...
void sockinetaddr::setaddr(const char* host_name)
{
  if ( (sin_addr.s_addr = inet_addr(host_name)) == INADDR_NONE) {
    ...
    if (hp == GDCM_NULLPTR) throw sockerr (EADDRNOTAVAIL, "sockinetaddr::setaddr");
...
bool sockinetbuf::tcpnodelay () const
{
  ...
  if (proto == GDCM_NULLPTR) throw sockerr (ENOPROTOOPT, "sockinetbuf::tcpnodelay");
...
isockinet::isockinet (const sockbuf::sockdesc& sd)
  : std::ios(GDCM_NULLPTR), isockstream(GDCM_NULLPTR)
{
...
osockinet::osockinet (const sockbuf::sockdesc& sd)
  : std::ios (GDCM_NULLPTR), osockstream(GDCM_NULLPTR)
{
    sockinetbuf *t = new sockinetbuf (sd);
...
iosockinet::iosockinet (const sockbuf::sockdesc& sd)
  : std::ios (GDCM_NULLPTR), iosockstream(GDCM_NULLPTR)
{
    sockinetbuf *t = new sockinetbuf(sd);
